...
function parseExcelDynamic(rawArray, keyIdentifiers) {
  ...
  for (let i = headerIdx + 1; i < rawArray.length; i++) {
    ...
    if (
      ...
      row.length === 0 ||
      !row.some((c) => c) ||
      str(row[0]).toUpperCase() === "TOTAL"
    )
      continue;
    const obj = {};
    headers.forEach((colName, idx) => {
      if (colName) obj[colName] = row[idx];
    });
    data.push(obj);
  }
  return data;
...
function findInv(invData, prodCode, modelName) {
  ...
  if (base.length > 5) {
    hit = invData.find(
      (r) =>
        r.품번?.toUpperCase().startsWith(base) ||
        r.품명?.toUpperCase().startsWith(base)
    );
    if (hit) return hit;
  }
  if (cModel.length > 3) {
    hit = invData.find(
      (r) =>
        r.품명?.toUpperCase().includes(cModel) ||
        r.품번?.toUpperCase().includes(cModel)
    );
    if (hit) return hit;
  }
  return null;
...
function DropZone({ label, icon, accept, onFile, loaded, fileName }) {
  ...
  return (
    <div
      ...
      style={{
        border: `2px dashed ${
          drag ? "#3b82f6" : loaded ? "#4ade80" : "#cbd5e1"
        }`,
        borderRadius: 12,
        padding: "22px 16px",
        textAlign: "center",
        cursor: "pointer",
        background: drag ? "#eff6ff" : loaded ? "#f0fdf4" : "#f8fafc",
        transition: "all .2s",
      }}
    >
      <input
...
export default function App() {
  ...
  const [prodFile, setProdFile] = useState(
    localStorage.getItem("wg_prodFile") || ""
  );
  const [invFile, setInvFile] = useState(
    localStorage.getItem("wg_invFile") || ""
  );
  ...
  const handleProdFile = useCallback((file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (mapped.length > 0) {
          setProdData(mapped);
          setParseMsg(`✅ 생산계획 ${mapped.length}건 로드 완료`);
        } else {
          setParseMsg("⚠️ 생산계획 데이터를 찾을 수 없습니다.");
        }
      } catch (err) {
        setParseMsg("❌ 생산계획 파싱 오류: " + err.message);
      }
    };
  ...
  const handleInvFile = useCallback((file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const mapped = parsed
          .map((r) => ({
            품번: str(r["품번"] || r["ITEM_CODE"]),
            품명: str(r["품명"] || r["ITEM_NAME"]),
            규격: str(r["규격"] || r["SPEC"]),
            재고수량: num(r["재고수량"] || r["수량"]),
        ...
        if (mapped.length > 0) {
          ...
          setParseMsg("⚠️ 재고현황 데이터를 찾을 수 없습니다.");
        }
      } catch (err) {
        setParseMsg("❌ 재고현황 파싱 오류: " + err.message);
      }
    };
    reader.readAsArrayBuffer(file);
  }, []);
  ...
  const handleShipParse = () => {
    ...
    if (rows.length === 0) {
      setParseMsg("⚠️ 유효한 출하의뢰 데이터가 없습니다.");
      return;
    }
    setShipData(rows);
    ...
    () =>
      prodData.map((r) => {
        ...
        return {
          ...r,
          _inv: inv,
          _status: shipStatus(inv?.재고수량 ?? null, r.수량),
        };
      }),
    [prodData, invData]
  );
  ...
  const shipDates = useMemo(() => {
    const m = {};
    prodEnriched.forEach((r) => {
      if (r.출하일자 && r.출하일자.startsWith("202"))
        m[r.출하일자] = (m[r.출하일자] || 0) + r.수량;
    });
  ...
  const filteredProd = useMemo(() => {
    ...
    return prodEnriched.filter((r) => {
      const mQ =
        !q ||
        r.고객명?.toLowerCase().includes(q) ||
        r.모델명?.toLowerCase().includes(q) ||
        r.제품코드?.toLowerCase().includes(q);
  ...
  const negInvList = useMemo(
    () => invData.filter((r) => r.재고수량 < 0),
    [invData]
  );
  ...
  const InputView = () => (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "20px 16px" }}>
      ...
      {parseMsg && (
        <div
          style={{
            background: parseMsg.startsWith("✅")
              ? "#f0fdf4"
              : parseMsg.startsWith("⚠️")
              ? "#fefce8"
              : "#fef2f2",
            border: `1px solid ${
              parseMsg.startsWith("✅")
      ...
      >
        {[
          ...
          },
          {
            key: "shortage",
            label: "❌ 재고부족",
            cnt: prodStats.shortage,
            color: "#991b1b",
          },
          {
            key: "unknown",
            label: "⚠️ 미확인",
            cnt: prodStats.unknown,
            color: "#713f12",
          },
        ].map((s) => (
          <button
            ...
            style={{
              ...
              background:
                filterStatus === s.key
                  ? STATUS[s.key]?.bg || "#f1f5f9"
                  : "#f8fafc",
              border: `1.5px solid ${
                filterStatus === s.key ? s.color + "55" : "#e2e8f0"
              }`,
              borderRadius: 8,
              padding: "5px 13px",
              cursor: "pointer",
      ...
      >
        {[
          { key: "prod", label: "📋 생산계획" },
          {
            key: "ship",
            label: "🚚 출하의뢰",
            extra: shipEnriched.filter((r) => r._status !== "ok").length,
          },
          ...
          <button
            ...
            style={{
              ...
              color: mainTab === t.key ? "#1d4ed8" : "#94a3b8",
            }}
          >
            {t.label}
            {t.extra > 0 && (
              <span
                style={{
                  marginLeft: 5,
                  background: t.key === "inv" ? "#ef4444" : "#f59e0b",
                  color: "#fff",
                  borderRadius: 9,
                  padding: "1px 6px",
                  fontSize: 10,
              ...
              >
                ...
                <tbody>
                  {filteredProd.map((r, i) => (
                    ...
                    >
                      <td style={TD}>
                        <ShipBadge status={r._status} />
                      </td>
                      <td style={{ ...TD, fontWeight: 700, color: "#1d4ed8" }}>
                        {fmtD(r.출하일자)}
                      </td>
                      <td style={{ ...TD, fontWeight: 600 }}>{r.고객명}</td>
                      <td style={TD}>{r.모델명}</td>
                      <td
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15 }}>출하 일정관리</div>
          </div>
        </div>
        <button
          onClick={() => {
